...
def main():
    ...
    #                              lr_lambda=lr_lambda, metrics=metrics)
    # cross_val_experiment(data_module=data_module_capgmyo, partial_classifier=partial_classifier,
    #                      name="150 Series Chinese CapgMyo", max_epochs=28, callbacks=callbacks, seed=seed,
    #                      model_checkpoint_index=0)

    callbacks = [partial(ModelCheckpoint, monitor='val/loss', dirpath=MODELS_FOLDER),
                 partial(EarlyStopping, monitor='val/Accuracy', patience=14, mode='max')]
    #
    # partial_classifier = partial(Classifier, optim_kwargs={'lr': 0.001, 'weight_decay': 0.0001}, monitor='val/Accuracy',
    #                              sched_kwargs={'patience': 7, 'mode': 'max'}, metrics=metrics)
    # cross_val_experiment(data_module=data_module_capgmyo, partial_classifier=partial_classifier, name="150 Series CapgMyo",
    ...
    #
    # partial_classifier = partial(Classifier, optimizer=optimizer, lr_scheduler=lr_scheduler,
    #                              optim_kwargs={'lr': 0.001, 'weight_decay': 0.0001}, time_window=[111], time_step=[1],
    #                              window_fix=[39], monitor='val/Accuracy', lr_lambda=lr_lambda, metrics=metrics)
    # cross_val_experiment(data_module=data_module_capgmyo, partial_classifier=partial_classifier,
    #                      name="40 Series Chinese CapgMyo", max_epochs=28, callbacks=callbacks, seed=seed,
    #                      model_checkpoint_index=0)
    #
    # callbacks = [partial(ModelCheckpoint, monitor='val/loss', dirpath=MODELS_FOLDER),
    #              partial(EarlyStopping, monitor='val/Accuracy', patience=14, mode='max')]
    #
    # partial_classifier = partial(Classifier, optim_kwargs={'lr': 0.001, 'weight_decay': 0.0001}, monitor='val/Accuracy',
    #                              sched_kwargs={'patience': 7, 'mode': 'max'}, time_window=[111], time_step=[1],
    #                              window_fix=[39], metrics=metrics)
    # cross_val_experiment(data_module=data_module_capgmyo, partial_classifier=partial_classifier,
    #                      name="40 Series CapgMyo",
    #                      max_epochs=150, callbacks=callbacks, seed=seed, model_checkpoint_index=0)
    #
    #
    ...
    #                      name="40 Series MyoArmband", max_epochs=150, callbacks=callbacks, seed=seed,
    #                      model_checkpoint_index=0)
    ...
    partial_classifier = partial(Classifier, optim_kwargs={'lr': 0.001, 'weight_decay': 0.0001}, monitor='val/Accuracy',
                                 sched_kwargs={'patience': 7, 'mode': 'max'}, metrics=metrics)
    cross_val_experiment(data_module=data_module_ninapro, partial_classifier=partial_classifier,
                         name="28 Series NinaPro", max_epochs=150, callbacks=callbacks, seed=seed,
                         model_checkpoint_index=0)
