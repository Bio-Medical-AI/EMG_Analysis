...
from datasets import prepare_knibm_high, prepare_dataframe_dataset, AbstractDataModule, SequenceDataset
from definitions import PKL_FOLDER


class KNIBMHighDataModule(AbstractDataModule):
    def __init__(self,
        ...
        """
        df_path = os.path.join(PKL_FOLDER, 'knibm-high', 'knibm-high.pkl')
        if not os.path.isfile(df_path):
            prepare_knibm_high(prepare_dataframe_dataset, PKL_FOLDER)
        super(KNIBMHighDataModule, self).__init__(
            df_path,
            window_length,
            8,
            1,
